"""
链表：
    也是一种数据结构，内存地址是链式的，后一个内存地址的位置与前一个内存地址是相互连接的，
    也就是说，链表里的元素储存了储存了下一个元素的内存地址，想知道一个元素的地址，就必须知道上一个元素的地址。
    因此，链表进行的读取操作是顺序访问。
    当执行读取操作时，可以直接对地址进行访问，运行时间：O(N)
    当进行插入操作时，需将后面的数据往后移动一格，运行时间：O(1)
    当进行删除操作时，需将前面的数据往前移动一格,运行时间：O(1)
"""
